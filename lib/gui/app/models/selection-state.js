...
exports.getSelectedVersion = () => {
  return store.getState().getIn([ 'selection', 'version' ]).toJS()
}

exports.hasVersion = () => {
  return !_.isEmpty(exports.getSelectedVersion())
}

exports.isVersionSelected = (version) => {
  if (!version) {
    return false
  }
  const selectedVersion = exports.getSelectedVersion()
  return selectedVersion.assets_url === version.assets_url
}

exports.selectVersion = (version) => {
  store.dispatch({
    type: store.Actions.SELECT_VERSION,
    data: version
  })
}

exports.hasBoard = () => {
  return !_.isEmpty(exports.getSelectedBoard())
}

exports.getSelectedBoard = () => {
  return store.getState().getIn([ 'selection', 'board' ])
}

exports.selectBoard = (board) => {
  store.dispatch({
    type: store.Actions.SELECT_BOARD,
    data: board
  })
}

exports.isBoardSelected = (board) => {
  if (!board) {
    return false
  }
  const b = exports.getSelectedBoard()
  return b === board
}


exports.hasConnection = () => {
  const connection = exports.getSelectedConnection()
  if(connection){
    if(connection === "Wireless") {
      const ssid =  exports.getSelectedSSID()
      if(ssid && ssid !== "") {
        if(ssid === "SSID is not in the list") {
          const custom_ssid = exports.getCustomSSID();
          const password_ssid = exports.getPasswordSSID();
          return custom_ssid && custom_ssid !== "" && password_ssid && password_ssid !== ""
        } else {
          const password_ssid = exports.getPasswordSSID();
          return password_ssid && password_ssid !== ""
        }
      }
      return false
    }
...
exports.selectConnection = (connection) => {
  store.dispatch({
    type: store.Actions.SELECT_CONNECTION,
    data: connection
  })
}

exports.getSelectedConnection = () => {
  return store.getState().getIn([ 'selection', 'connection' ])
}

exports.getSelectedSSID = () => {
  return store.getState().getIn([ 'selection', 'ssid' ])
}

exports.selectSSID = (ssid) => {
  store.dispatch({
    type: store.Actions.SELECT_SSID,
    data: ssid
  })
}

exports.getCustomSSID = () => {
  return store.getState().getIn([ 'selection', 'custom_ssid' ])
}

exports.setCustomSSID = (ssid) => {
  store.dispatch({
    type: store.Actions.SET_CUSTOM_SSID,
    data: ssid
  })
}

exports.getPasswordSSID = () => {
  return store.getState().getIn([ 'selection', 'password' ])
}

exports.setPassword = (password) => {
  store.dispatch({
    type: store.Actions.SET_PASSWORD_SSID,
    data: password
  })
}

exports.selectNetworkConfig = (networkconfig) => {
  store.dispatch({
    type: store.Actions.SELECT_NETWORK_CONFIG,
    data: networkconfig
  })
}

exports.getSelectedNetworkConfig = () => {
  return store.getState().getIn([ 'selection', 'network_config' ])
}

exports.selectIP = (ip) => {
  store.dispatch({
    type: store.Actions.SELECT_IP,
    data: ip
  })
}

exports.getSelectedIP = () => {
  return store.getState().getIn([ 'selection', 'ip_address' ])
}

exports.selectGateway = (gateway) => {
  store.dispatch({
    type: store.Actions.SELECT_GATEWAY,
    data: gateway
  })
}

exports.getSelectedGateway = () => {
  return store.getState().getIn([ 'selection', 'ip_gateway' ])
}

exports.selectDNS = (dns) => {
  store.dispatch({
    type: store.Actions.SELECT_DNS,
    data: dns
  })
}

exports.getSelectedDNS = () => {
  return store.getState().getIn([ 'selection', 'ip_dns' ])
}

exports.hasNetworkConfig = () => {
  const network_config = exports.getSelectedNetworkConfig()
  if(network_config){
    if(network_config === "Static IP") {
      const ip =  exports.getSelectedIP()
      const gateway = exports.getSelectedGateway();
      const dns = exports.getSelectedDNS();
