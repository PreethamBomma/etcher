...
/**
 * @summary Get available versions from the state
 * @function
 * @public
 *
 * @param {Object} state - state object
 ...
 *
 * @example
 * const drives = getAvailableVersions(state)
 * _.find(drives, { device: '/dev/sda' })
 */
const getAvailableVersions = (state) => {
  // eslint-disable-next-line lodash/prefer-lodash-method
  return state.get('availableVersions').toJS()
}
...
const storeReducer = (state = DEFAULT_STATE, action) => {
  switch (action.type) {
    ...
    case ACTIONS.SET_AVAILABLE_VERSIONS: {
      // Type: action.data : Array<DriveObject>
      ...
      const newState = state.set('availableVersions', Immutable.fromJS(versions))
      return newState
    }


    case ACTIONS.SET_AVAILABLE_SSIDS: {
      if (!action.data) {
        throw errors.createError({
          title: 'Missing SSIDS'
        })
      }
      const ssids = action.data
      const newState = state.set('availableSSIDS', Immutable.fromJS(ssids))
      return newState
    }
    ...
    case ACTIONS.SELECT_VERSION: {
      ...
      for(let i = 0; i < assets.length; i++){
        ...
        if(asset.browser_download_url){
          const chunks = asset.browser_download_url.split('/')
          if(chunks[chunks.length-1]) {
            const chunksFile = chunks[chunks.length-1].split('-')
            boards.push(chunksFile[1])
          }
        }
      }
      return state.setIn([ 'selection', 'version' ], Immutable.fromJS(action.data))
             .set('availableBoards', Immutable.fromJS(boards))
    }

    case ACTIONS.SELECT_SSID: {
      const ssid = action.data
      return state.setIn([ 'selection', 'ssid' ], Immutable.fromJS(ssid))
    }

    case ACTIONS.SELECT_BOARD: {
      const board = action.data
      return state.setIn([ 'selection', 'board' ], Immutable.fromJS(board))
    }

    case ACTIONS.SELECT_CONNECTION: {
      const connection = action.data
      return state.setIn([ 'selection', 'connection' ], Immutable.fromJS(connection))
    }

    case ACTIONS.SET_CUSTOM_SSID: {
      const ssid = action.data
      return state.setIn([ 'selection', 'custom_ssid' ], Immutable.fromJS(ssid))
    }

    case ACTIONS.SET_PASSWORD_SSID: {
      const password = action.data
      return state.setIn([ 'selection', 'password' ], Immutable.fromJS(password))
    }

    case ACTIONS.SELECT_NETWORK_CONFIG: {
      const network_config = action.data
      return state.setIn([ 'selection', 'network_config' ], Immutable.fromJS(network_config))
    }

    case ACTIONS.SELECT_IP: {
      const ip_address = action.data
      return state.setIn([ 'selection', 'ip_address' ], Immutable.fromJS(ip_address))
    }

    case ACTIONS.SELECT_GATEWAY: {
      const ip_gateway = action.data
      return state.setIn([ 'selection', 'ip_gateway' ], Immutable.fromJS(ip_gateway))
    }

    case ACTIONS.SELECT_NETWORK_CONFIG: {
      const ip_dns = action.data
      return state.setIn([ 'selection', 'ip_dns' ], Immutable.fromJS(ip_dns))
    }
