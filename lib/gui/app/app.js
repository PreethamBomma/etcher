...
app.run(() => {
  //setInterval(() => {
    ...
    const url = "https://api.github.com/repos/kerberos-io/kios/releases?client_id=d94be23a92a1947d19f5&client_secret=7bf76002079fbdb07992a9bc5c4f57a8dda0c64a?client_id=d94be23a92a1947d19f5&client_secret=7bf76002079fbdb07992a9bc5c4f57a8dda0c64a"
    fetch(url)
    .then(res => res.json())
    .then(versions => {
  ...
  setInterval(() => {
    scanner.scan((err, networks) => {
      ...
      for(let i = 0; i < networks.length; i++) {
        ...
        if(network && !ssids.includes(network.ssid)) {
          ssids.push(network.ssid)
        }
      }
      availableVersions.setAvailableSSIDS(ssids)
    });
  }, 2500);
})
