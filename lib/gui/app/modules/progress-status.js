...
exports.fromFlashState = (state) => {
  ...
  const isFlashing = !isDownloading && Boolean(state.flashing)
  ...
  if (isDownloading) {
    if(state.percentage === utils.PERCENTAGE_MINIMUM){
      return 'Downloading...'
    }
    return `${state.percentage}% Downloading`
  } else if (state.percentage === utils.PERCENTAGE_MINIMUM && !state.speed) {
    if (isValidating) {
      return 'Validating...'
    }
    return 'Starting...'
  ...
  } else if (!isFlashing && !isValidating && !isDownloading) {
    return 'Failed'
  }
