...
exports.flash = (image, drives) => {
  ...
  const network = "wifi" // eth
  const ssid = "telenet-10D7F"
  const ssid_password = "blablabla"

  const ip = "static" // dynamic
  const ipaddress = "192.168.0.12"
  const ipgateway = "192.168.0.1"
  const ipdns = "8.8.8.8"

  const board = "raspberrypi3"
