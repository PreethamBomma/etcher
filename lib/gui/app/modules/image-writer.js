...
exports.flash = (image, drives) => {
  ...
  var fileUnzipped = tmp.fileSync();

  const connection = store.getState().toJS().selection.connection
  const ssid = store.getState().toJS().selection.ssid
  const custom_ssid = store.getState().toJS().selection.custom_ssid
  const password = store.getState().toJS().selection.password

  const network_config = store.getState().toJS().selection.network_config
  const ip_address = store.getState().toJS().selection.ip_address
  const ip_gateway = store.getState().toJS().selection.ip_gateway
  const ip_dns = store.getState().toJS().selection.ip_dns

  const board = store.getState().toJS().selection.board
  ...
  return new Bluebird((resolve, reject) => {
    ...
    if(connection === "Wireless") {
      ...
      network={
          scan_ssid=1
          ssid="${custom_ssid ? custom_ssid : ssid}"
          psk="${password}"
      }`);
    }
    return
